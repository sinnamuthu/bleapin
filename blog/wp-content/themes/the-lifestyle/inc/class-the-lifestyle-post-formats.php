...
class The_Lifestyle_Post_Formats {
  ...
  public function __construct() {
    ...
    $this->formats = array( 'gallery', 'video', 'audio' );
  }

  public function setup() {
		add_action( 'the_lifestyle_format_content', array( $this, 'format_content' ), 10, 4 );
  ...
  public function format_content( $args = array(), $remove = true, $id = 0 ) {
          ...
          if( isset( $first_gallery_shortcode_atts['ids'] ) ) {
            ...
            if( $gallery_ids ) {
              foreach( $gallery_ids as $image_id ) {
                $format_output .= sprintf( '<div class="gallery-image">%s</div>', wp_get_attachment_image( $image_id, $args['images_size'] ) );
              }
            }
          }
    ...
    if( $format =='video' ) {
      printf( '<div class="entry-format entry-format-%s wp-block-embed wp-embed-aspect-16-9 wp-has-aspect-ratio"><div class="wp-block-embed__wrapper">%s %s %s</div></div>', $format, $args['before'], $format_output, $args['after'] );
    } else {
...
$the_lifestyle_post_formats = new The_Lifestyle_Post_Formats();

add_action( 'after_setup_theme', array( $the_lifestyle_post_formats, 'setup' ), 15 );

if ( ! function_exists( 'the_lifestyle_format_content' ) ) :
  /**
   * Prints HTML with post format specific content.
   *
   * @since The Lifestyle 1.1
   */
  function the_lifestyle_format_content( $args = array(), $remove = true,  $id = 0 ) {
    do_action( 'the_lifestyle_format_content', $args, $remove, $id );
  }
endif;
?>